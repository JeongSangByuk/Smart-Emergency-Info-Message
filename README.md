...
+ 재난문자가 메인 액티비티 안에서 사용자의 설정(위치, 카테고리 선택 유무, 카테고리 가중치)에 따라 동적으로 처리되면서, 메인 액티비티의 재난문자 리사이클러뷰에 대한 아주 섬세한 처리가 필요했습니다. 이 과정에서 발생한 수 많은 예외들을 처리하고 재난문자와 관련된 모든 value들을 고려하면서 기능의 로직을 개발하기 위해 많은 시간을 사용했습니다.
